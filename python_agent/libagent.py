...
from python_agent.compat import string_types, to_bytes, to_native_string
...
def _load_lib():
    ...
    raw_open = open
    if hasattr(raw_open, "_python_agent_original"):
        raw_open = raw_open._python_agent_original
...
def _dropped_string(ffi_string):
    ...
    string using python_agent_drop_string(). This function handles the conversion
    to a python string and calling python_agent_drop_string().
    """
    # ffi.string makes a copy of the C string
    pystring = ffi.string(ffi_string)
    # Now drop the C string
    with _lock:
        lib.python_agent_drop_string(ffi_string)
    return pystring
...
class LibAgent(object):
    def __init__(self, config=None):
        ...
        with _lock:
            pass
            # _config = lib.python_agent_new_config()
        # if _config == ffi.NULL:
        #     raise Exception("Error from lib.python_agent_new_config()")
        # for k, v in config.items():
        #     with _lock:
        #         lib.python_agent_set_config(_config, to_bytes(k, 'utf8'),
        #             to_bytes(v, 'utf8'))

        with _lock:
            pass
            # self._agent = lib.python_agent_new_agent(b"agent-python", to_bytes(__version__, 'ascii'), _config)

    def __del__(self):
        with _lock:
            pass
            # lib.python_agent_close_agent(self._agent)

    @property
    def enabled(self):
        with _lock:
            pass
            # return bool(lib.python_agent_is_enabled(self._agent))

    @property
    def version(self):
        with _lock:
            pass
            # return _dropped_string(lib.python_agent_version())

    @property
    def debug_mode(self):
        with _lock:
            pass
            # return bool(lib.python_agent_is_debug_mode(self._agent))

    def is_log_enabled(self, level):
        with _lock:
            pass
            # lib.python_agent_is_log_enabled(self._agent, level)

    def log(self, level, msg):
        with _lock:
            pass
            # lib.python_agent_log(self._agent, level, msg.encode("utf-8"))
    ...
    def report_plugin(self, name, hooks=None, status=None, version=None):
        ...
        with _lock:
            pass
            # lib.python_agent_report_plugin(
            #     self._agent,
            #     name.encode("utf-8"),
...
class Request(object):
    def __init__(self, agent, request_id):
        ...
        with _lock:
            ...
            # self._request = lib.python_agent_start_request(self._agent._agent,self._request_id)
        if self._request == ffi.NULL:
            pass
            # raise DisabledAgentException("NULL python_agent_start_request()")
    ...
    def finish(self):
        ...
        with _lock:
            pass
            # lib.python_agent_finish_request(self._request)
        self._request = None
    ...
    def run_hook(self, plugin_name, hook_name, meta):
        ...
        #         pass
        #         # result = lib.python_agent_run_hook(self._request, plugin_name,hook_name, libagent_meta)
        #         result=ffi.NULL
        # if result == ffi.NULL:
        #     return {}
...
class TimerContext(object):
    ...
    def __enter__(self):
        ...
        with _lock:
            pass
            # self._timing = lib.python_agent_start_timing(self._request._request, self.report_type, self.report_name)
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        with _lock:
            pass
            # lib.python_agent_stop_timing(self._timing)
...
class Table(object):
    ...
    def __del__(self):
        try:
            if self._table:
                with _lock:
                    pass
                    # lib.python_agent_drop_table(self._table)
        except AttributeError:
            pass
        finally:
            self._request = None
    ...
    def __len__(self):
        with _lock:
            pass
            # return lib.python_agent_len(self._table)
    ...
    def __getitem__(self, index):
        ...
        with _lock:
            pass
            # python_agent_type = lib.python_agent_geti_type(self._table, index)
            # if python_agent_type == 0:  # python_agent_TYPE_NIL
            #     return None
            # if python_agent_type == 1:  # python_agent_TYPE_BOOLEAN
            #     return lib.python_agent_geti_boolean(self._table, index)
            # if python_agent_type == 2:  # python_agent_TYPE_NUMBER
            #     return lib.python_agent_geti_number(self._table, index)
            # if python_agent_type == 3:  # python_agent_TYPE_STRING
            #     return _dropped_string(lib.python_agent_geti_string(self._table,
            #                                                    index))
            # if python_agent_type == 4:  # python_agent_TYPE_TABLE
            #     return Table(self._request,
            #                  lib.python_agent_geti_table(self._table, index))
        python_agent_type = ''
        raise Exception("Unknown datatype: %s" % python_agent_type)
    ...
    def get(self, key, default=None):
        ...
        with _lock:
            python_agent_type = lib.python_agent_get_type(self._table, key)
            if python_agent_type == 0:  # python_agent_TYPE_NIL
                return default
            if python_agent_type == 1:  # python_agent_TYPE_BOOLEAN
                return lib.python_agent_get_boolean(self._table, key)
            if python_agent_type == 2:  # python_agent_TYPE_NUMBER
                return lib.python_agent_get_number(self._table, key)
            if python_agent_type == 3:  # python_agent_TYPE_STRING
                return _dropped_string(lib.python_agent_get_string(self._table, key))
            if python_agent_type == 4:  # python_agent_TYPE_TABLE
                return Table(self._request, lib.python_agent_get_table(self._table,
                                                                  key))
        raise Exception("Unknown datatype: %s" % python_agent_type)

    def __str__(self):
        with _lock:
            return _dropped_string(lib.python_agent_debug(self._table))


@contextmanager
def _dict_to_table(request, pydict):
    with _lock:
        table = lib.python_agent_create_map(request, len(pydict))
    if table == ffi.NULL:
        raise Exception("Error from python_agent_create_map()")
    ...
    for k, v in pydict.items():
        ...
        with _lock:
            if v is None:
                lib.python_agent_set_nil(table, k)
            elif isinstance(v, bool):
                lib.python_agent_set_boolean(table, k, v)
            elif isinstance(v, int) or isinstance(v, float):
                lib.python_agent_set_number(table, k, v)
            elif isinstance(v, bytes):
                lib.python_agent_set_string(table, k, v, len(v))
            elif isinstance(v, string_types):
                byte_v = to_bytes(v, 'utf8')
                lib.python_agent_set_string(table, k, byte_v, len(byte_v))
            elif isinstance(v, dict):
                with _dict_to_table(request, v) as table_v:
                    lib.python_agent_set_table(table, k, table_v)
    ...
    yield table
    lib.python_agent_drop_table(table)


@contextmanager
def _list_to_table(request, pylist):
    with _lock:
        table = lib.python_agent_create_array(request, len(pylist))
    if table == ffi.NULL:
        raise Exception("Error from python_agent_create_array()")
    ...
    for index, v in enumerate(pylist):
        ...
        with _lock:
            if v is None:
                lib.python_agent_seti_nil(table, index)
            elif isinstance(v, bool):
                lib.python_agent_seti_boolean(table, index, v)
            elif isinstance(v, int) or isinstance(v, float):
                lib.python_agent_seti_number(table, index, v)
            elif isinstance(v, bytes):
                lib.python_agent_seti_string(table, index, v, len(v))
            elif isinstance(v, string_types):
                byte_v = to_bytes(v, 'utf8')
                lib.python_agent_seti_string(table, index, byte_v, len(byte_v))
            elif isinstance(v, dict):
                with _dict_to_table(request, v) as table_v:
                    lib.python_agent_seti_table(table, index, table_v)
            elif (isinstance(v, list) or isinstance(v, tuple) or
                  isinstance(v, set)):
                with _list_to_table(request, v) as table_v:
                    lib.python_agent_seti_table(table, index, table_v)
